...
impl<P: ThreadPool> KvsEngine for KvStore<P> {
    ...
    /// Scan all string value of a given key.
    ///
    /// Returns `None` if the given key does not exist.
    fn scan(&self, key: String) -> Result<Vec<String>> {
        let re = Regex::new(&key).map_err(|e| KvsError::StringError(e.to_string()))?;
        Ok(self.index.iter().filter(|e| {
            re.is_match(e.key())
        }).map(|e| e.key().clone()).collect::<Vec<String>>())
    }
